...
/// Simd JSON Extractor / Response. Copy of axum's Json extractor.
///
    ...
    S: Send + Sync,
{
    ...
    async fn from_request(req: Request, state: &S) -> Result<Self, Self::Rejection> {
        if json_content_type(req.headers()) {
            ...
            let r = MissingJsonContentType::default();
            Err(r.into())
        }
    }
}
    ...
    T: for<'a> Deserialize<'a>,
{
    ...
    pub fn from_bytes(bytes: &mut [u8]) -> Result<Self, JsonRejection> {
        let value = match T::from_slice(bytes) {
            Ok(value) => value,
            Err(_err) => {
                // FIXME: why can't we have nice errors?>?:
                let rejection = MissingJsonContentType::default().into();
    ...
    T: Serialize,
{
    fn into_response(self) -> Response {
        ...
        match self.0.json_write(&mut buf) {
            Ok(()) => (
                [(
                    header::CONTENT_TYPE,
...
// mod tests {
//     use super::*;
//     use axum::{routing::post, test_helpers::*, Router};
